...
//
//      // Spawn factory is ObjectInitializer-based (TLS-backed):
//      // The factory pushes Init into TLS and calls the default ctor.
//      // Only emitted when valid (not abstract, and not user-disabled).
...
//      - "Abstract"      if std::is_abstract_v<Self> is true
//      - "NoSpawn"       if it has no default ctor (TLS-backed spawn requires default construction)
//
//    NOTE:
...
fn parse_members(class_name: &str, body: &[Token]) -> (Vec<PropertyInfo>, Vec<FunctionInfo>) {
    ...
    while i < body.len() {
        ...
        // Detect ctor without JFUNCTION (default or overloaded)
        if body[i].kind == TokKind::Ident && body[i].text == class_name {
            if let Some((ctor, new_i)) =
                parse_ctor_without_marker(body, i, class_name, current_access.clone())
            {
                funcs.push(ctor);
                i = new_i;
                continue;
...
fn parse_ctor_without_marker(
    tokens: &[Token],
    mut i: usize,
    class_name: &str,
    access: Option<String>,
) -> Option<(FunctionInfo, usize)> {
    // Expect: ClassName ( ... ) tail ;  OR  ClassName ( ... ) tail { ... }

    if i >= tokens.len() || tokens[i].kind != TokKind::Ident || tokens[i].text != class_name {
        return None;
    }
...
fn emit_generated_h_for_class(class_name: &str, super_name: &str) -> String {
    ...
    out.push_str("#undef GENERATED_BODY\n");
    out.push_str("#define GENERATED_BODY() \\\n");
    out.push_str("public: \\\n");
...
fn emit_generated_cpp_for_class(header_path: &str, class: &ClassInfo) -> String {
    ...
    out.push_str("#include \"Core/FObjectInitTLS.h\"\n");
    out.push_str("#include \"Core/Reflection/RETypeRegistry.h\"\n\n");
    ...
    for f in &class.functions {
        // Do not register ctors/dtors as functions (ctors are TLS-backed via factory)
        if f.return_ty.is_none()
            && (f.name == *class_name || f.name == format!("~{}", class_name))
        {
            continue;
        }
    ...
    // ---------------- Factory analysis (TLS-backed spawn) ----------------
    ...
    if !has_default_ctor {
        ...
        out.push_str(&format!(
            "                R.AddTypeMeta(typeid({}), \"NoSpawn\", \"\");\n",
            class_name
        ));
        out.push_str("            }\n");
    }

    // Factory only if NOT abstract, user didn't force-disable, and default ctor exists
    if !no_factory && has_default_ctor {
        out.push_str(&format!(
            "            if constexpr (!std::is_abstract_v<{}>) {{\n",
            class_name
        ));
        out.push_str(&format!(
            "                R.SetFactory(typeid({}), [](const FObjectInitializer& Init) -> JCoreObject* {{ FObjectInitTLS::FScope Scope(Init); return new {}(); }});\n",
